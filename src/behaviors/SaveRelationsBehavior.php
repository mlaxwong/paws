...
class SaveRelationsBehavior extends Behavior
{
    ...
    public function __set($name, $value)
    {
        ...
        if (in_array($name, $this->_relations)) {
            Paws::debug("Setting {$name} relation value", __METHOD__);
    ...
    protected function saveRelatedRecords(BaseActiveRecord $model, ModelEvent $event)
    {
        try {
            ...
            Paws::warning(get_class($e) . ' was thrown while saving related records during beforeValidate event: ' . $e->getMessage(), __METHOD__);
    ...
    {
        Paws::debug("_prepareHasOneRelation for {$relationName}", __METHOD__);
    ...
    protected function validateRelationModel($prettyRelationName, $relationName, BaseActiveRecord $relationModel)
    {
        ...
        if (!is_null($relationModel) && ($relationModel->isNewRecord || count($relationModel->getDirtyAttributes()))) {
            Paws::debug("Validating {$prettyRelationName} relation model using " . $relationModel->scenario . ' scenario', __METHOD__);
    ...
    protected function _setRelationForeignKeys($relationName)
    {
        ...
        if ($relation->multiple === false && !empty($owner->{$relationName})) {
            Paws::debug("Setting foreign keys for {$relationName}", __METHOD__);
    ...
    public function afterSave()
    {
        if ($this->_relationsSaveStarted === false) {
            ...
            try {
                foreach ($this->_relations as $relationName) {
                    if (array_key_exists($relationName, $this->_oldRelationValue)) { // Relation was not set, do nothing...
                        Paws::debug("Linking {$relationName} relation", __METHOD__);
                        /** @var ActiveQuery $relation */
                        $relation = $owner->getRelation($relationName);
                ...
                Paws::warning(get_class($e) . ' was thrown while saving related records during afterSave event: ' . $e->getMessage(), __METHOD__);
                $this->_rollback();
                /***
    ...
    public function afterDelete()
    {
        ...
        foreach ($this->_relationsToDelete as $modelToDelete) {
            try {
                ...
                Paws::warning(get_class($e) . ' was thrown while deleting related records during afterDelete event: ' . $e->getMessage(), __METHOD__);
                $this->_rollback();
                throw $e;
            }
        }
